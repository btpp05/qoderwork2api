...
func (c *Client) ChatForward(cred *cred.Cred, openaiMessages []map[string]any, modelKey string, clientTools []any) (io.ReadCloser, error) {
	if cred.DT == "" {
		return nil, &Error{Kind: ErrTokenExpired, Status: 0, Msg: "no dt- available"}
	}
	// 构造 body
	rawBody, err := BuildAgentBody(openaiMessages, modelKey, clientTools)
	if err != nil {
